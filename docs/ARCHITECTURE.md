...
| **toolregister** | 暴露给 LLM 的"注册工具"tool；LLM 经 tool_call 调用它来注册新 tool（包装 `Agent.register`） | 否 |
...
- **Agent 是唯一注册器**；`chat` / `chat_ui` / `toolregister` 都不是注册器。
...
- `chat_ui` 通过直接调用 `Agent.register/unregister` 管理开关；LLM 通过 `toolregister`（→ `Agent.register`）自扩展。
...
- `toolregister` 让 LLM 具备自扩展能力（安装新 tool），是内核极少却可生长的关键支点。
