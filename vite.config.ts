  ...
  plugins: [
    monkey({
      ...
      userscript: {
        ...
        // basement 核心经 @require 引入（当前版本，jsDelivr 按 tag 分发）：运行时仅绑定 executor↔agent，不自动 init；
        // 由本脚本的 gm_storage.register 触发 boot() 完成启动后，再挂载 GM_* 存储 / DOM UI，不打包任何核心源码。
        require: [
          basementUrl,
